    ...
    static function get_breadcrumbs($breadcrumbs_array) {
        ...
        foreach ($breadcrumbs_array as $key => $breadcrumb) {
            ...
            if (empty($breadcrumb['url'])) {
                ...
                //normal links
                if($breadcrumb['display_name'] == "Home"){
                if ($key != 0) { //add separator only after first
                    $buffy .= ' <i class="td-icon-right td-bread-sep"></i> ';
                }    
                $buffy .= '<span><a title="' . esc_attr( $breadcrumb['title_attribute'] ) . '" class="entry-crumb" href="https://www.mytechmag.com">' . esc_html( $breadcrumb['display_name'] ) . '</a></span>';
                ...
                if($breadcrumb['display_name'] == "General"){
                if ($key != 0) { //add separator only after first
                    $buffy .= ' <i class="td-icon-right td-bread-sep"></i> ';
                }    
                $buffy .= '<span><a title="' . esc_attr( $breadcrumb['title_attribute'] ) . '" class="entry-crumb" href="'.get_site_url().'">' . get_bloginfo() . '</a></span>';
                //$buffy .= '<span><a title="' . esc_attr( $breadcrumb['title_attribute'] ) . '" class="entry-crumb" href="' . esc_url( $breadcrumb['url'] ) . '">' . esc_html( $breadcrumb['display_name'] ) . '</a></span>';
                }
            }
